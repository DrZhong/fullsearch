...
namespace _51API.FullSearch.HELP
{
    ...
    {
        public static IEnumerable<string> SplitWords(string content)
        {
        ...
        #region 根据传过来的内容返回盘古分词
        /// <summary>
        /// 根据传过来的内容返回盘古分词
        /// </summary>
        /// <param name="content"></param>
        /// <returns></returns>
        public static string GetPanguWords(string content)
        {
            ...
            while ((token = tokenStream.Next()) != null)//reader.Read()//只要还有词，就不返回null
            {
                ...
                back += " "+token.TermText();//token.TermText()为当前分的词
            }
            return back;
        } 
        #endregion
    ...
    public static class DXHanZiToPinYin
    {
 ...
 { "A", "Ai", "An", "Ang", "Ao", "Ba", "Bai", "Ban", "Bang", "Bao", "Bei", 
     ...
     "Zui", "Zun", "Zuo" };
        ...
        public static string Convert(string hzString, int maxLength)
        {
            ...
            //字符处理
            hzString = hzString.Trim().Replace("?", "_").Replace("\\", "_").Replace("/", "_").Replace(":", "").Replace("*", "").Replace(">", "").Replace("<", "").Replace("?", "").Replace("|", "").Replace("\"", "'").Replace("(", "_").Replace(")", "_").Replace(";", "_");
